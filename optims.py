...
def get_beta(v, std = 1e-3):

    v_np = v.detach().cpu().numpy()[0:1] # we take one gradient vector - they all point to the same direction
    ns_basis = scipy.linalg.null_space(v_np)
    mean = np.zeros(v.shape[1])
    noise = np.random.normal(0, std, v.shape)
    ns_noise = noise @ ns_basis@ns_basis.T
    beta = torch.tensor(ns_noise) - v
...
def sgd(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if (not add_noise) or ( len(param.shape) == 1 ):
